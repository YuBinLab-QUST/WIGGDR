...
def row_normalized(SimMatrix):
    row_sums = np.sum(SimMatrix, axis=1)
    NormalizedSimMatrix = SimMatrix / row_sums[:, np.newaxis]
    return NormalizedSimMatrix
...
class DRDataset():
    def __init__(self, dataset_name="Fdataset", drug_neighbor_num=15, disease_neighbor_num=15):
        ...
        self.drug_sim = old_data["drug"].astype(np.float64)
        self.disease_sim = old_data["disease"].astype(np.float64)
        self.drug_sim=row_normalized(self.drug_sim)
        self.disease_sim=row_normalized(self.disease_sim)
