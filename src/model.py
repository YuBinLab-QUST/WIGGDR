...
class NeighborEmbedding(nn.Module):
    def __init__(self, num_embeddings, out_channels=128, dropout=0.5, cached=True, bias=True, lamda=0.8, share=True):
        ...
        self.wiggdr = WIGGDR(in_channels=num_embeddings, out_channels=out_channels,
    ...
    def forward(self, x, edge, embedding):
        ...
        embedding = self.wiggdr(embedding, edge_index=edge_index)
...
class InteractionEmbedding(nn.Module):
    ...
    def forward(self, association_pairs, drug_embedding, disease_embedding):
        ...
        drug_embedding = F.embedding(association_pairs[0, :],
                                     drug_embedding)
...
class DRModel(BaseModel):
    ...
    def add_model_specific_args(parent_parser):
        parser = parent_parser.add_argument_group("DRModel model config")
        ...
        parser.add_argument("--lamda", type=float, default=0.8, help="weight for wiggdr")
    ...
    def forward(self, interaction_pairs, drug_edge, disease_edge, drug_embedding, disease_embedding):
        drug_neighbor_embedding = self.drug_neighbor_encoder(interaction_pairs[0, :], drug_edge,
                                                             drug_embedding)
        disease_neighbor_embedding = self.disease_neighbor_encoder(interaction_pairs[1, :], disease_edge,
                                                                   disease_embedding)
        interaction_embedding = self.interaction_encoder(interaction_pairs, drug_embedding,
                                                         disease_embedding)

        embedding = torch.cat([drug_neighbor_embedding, interaction_embedding, disease_neighbor_embedding],
                              dim=-1)
