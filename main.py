...
def train(config, model_cls=DRModel):
    ...
    logger.info(
        f"final {config.dataset_name}-{config.split_mode}-{config.n_splits}-fold-auroc:{final_metric['auroc']}-aupr:{final_metric['aupr']}")
...
def parse(print_help=False):
    ...
    parser.add_argument("--drug_feature_topk", default=20, type=int)  
    ...
    parser = DRModel.add_model_specific_args(parser)
